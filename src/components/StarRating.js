      ...
      <div style={starContainerStyle}>
        {Array.from({ length: maxRating }, (_, i) => (
          <Star
            ...
            onSetHoverRating={() => handleSetHoverRating(i + 1)}
            onRemoveHoverRating={() => handleSetHoverRating(0)}
            isStarFull={hoverRating ? hoverRating >= i + 1 : rating >= i + 1}
            color={color}
            size={size}
          />
        ))}
...
function Star({
  ...
  return (
    <span
      ...
      onMouseLeave={onRemoveHoverRating}
    >
      {isStarFull ? (
        <svg
